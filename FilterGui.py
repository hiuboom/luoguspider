...
class FilterApp:
    ...
    def create_widgets(self):
        ...
        difficulty_options = ["","入门", "普及-", "普及/提高-", "普及+/提高", "提高+/省选-", "省选/NOI-", "NOI/NOI+/CTSC"]
        dropdown_difficulty = ttk.Combobox(frame, textvariable=self.selected_difficulty, values=difficulty_options)
        ...
        keywords_options = ["","NOIP 普及组", "NOIP 提高组", "递归", "字符串" ,"动态规划","枚举","高精度","模拟","02优化","进制"]
        dropdown_keywords = ttk.Combobox(frame, textvariable=self.selected_keywords, values=keywords_options)
        ...
        self.entry_input_keyword = tk.Entry(frame, textvariable=self.inputted_keyword)
        self.entry_input_keyword.grid(row=2, column=1, padx=10, pady=10)

        # 创建返回主界面按钮
        return_button = tk.Button(frame, text="返回主界面", command=self.return_to_main)
        return_button.grid(row=4, column=0, columnspan=2, padx=5, pady=5)

        # 创建提交筛选项按钮
        button_submit = tk.Button(frame, text="提交筛选项", command=self.show_selected)
        ...
        self.listbox_results = tk.Listbox(results_frame, height=8, width=40, yscrollcommand=scrollbar_results.set)
        self.listbox_results.pack(side=tk.LEFT, fill=tk.BOTH)
        self.listbox_results.bind("<<ListboxSelect>>", self.show_file)  # 点击列表框中的文件名，会在文本框中显示文件内容
    ...
    def show_selected(self):
        ...
        for P_data in Problem_data:
            ...
            selecttwo = target_keyword == "" or any(tag in target_keyword for tag in P_data["标签"])
    ...
    def show_file(self, event):
        selected_file = self.listbox_results.get(self.listbox_results.curselection())  # 获取用户选择的文件名
